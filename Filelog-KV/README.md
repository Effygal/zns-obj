...
# FilelogKV ![MIT](https://img.shields.io/badge/license-MIT-blue.svg) 
## Status

**[FilelogKV is a toy testbed for the zns-obj project](https://https://github.com/Effygal/zns-obj)**

## Overview

`FilelogKV`  is a two-layer (gateways and loggers) distributed KV storage system; built on top of the conventional file system, with the underlying file on each logger serving as the log for KV storage; data is stored in an append manner; we utilize the current LBA of the file as a weak version ID; providing both client, gateway server, and logger server implementations. `FilelogKV` depends on the rpclib of C++, built with C++17.
